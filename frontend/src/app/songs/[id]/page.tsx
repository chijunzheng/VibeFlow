...
import { fetchSong, generateVibe, getStreamLyricsUrl, countSyllables, analyzeStress, rewriteText, updateSong, brainstormVibes, refineLyrics, Song } from "@/lib/api";
import { Loader2, Sparkles, PenTool, Activity, Edit2, Check, X, Copy, CheckCircle2, RotateCcw, Trash, Plus } from "lucide-react";
...
export default function SongEditor({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [vibeCandidates, setVibeCandidates] = useState<string[]>([]);
  ...
  const handleToggleVibeCandidate = async (candidate: string) => {
    if (!song) return;
    const currentVibes = song.vibe_cloud || [];
    let newVibes;
    if (currentVibes.includes(candidate)) {
      newVibes = currentVibes.filter(v => v !== candidate);
    } else {
      newVibes = [...currentVibes, candidate];
    }

    // Optimistic update
    setSong({ ...song, vibe_cloud: newVibes });

    try {
      await updateSong(song.id, { vibe_cloud: newVibes });
    } catch (err) {
      toast("Failed to update vibes", "error");
    }
  };
  ...
  const handleRewrite = async () => {
    ...
    setRewriting(true);
    try {
      const result = await refineLyrics(song.id, lyrics, selection, instructions);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 flex-1 min-h-0">
        ...
        <div className="lg:col-span-1 bg-slate-900/40 backdrop-blur-md rounded-xl border border-slate-800/50 p-4 flex flex-col gap-4 overflow-y-auto shadow-xl">
          ...
          <div className="space-y-2">
            <input
              type="text"
              placeholder="Seed phrase (e.g., Rain)"
            ...
            >
              {generatingVibe ? "Brainstorming..." : "Brainstorm Vibes"}
            </button>
          </div>

          {vibeCandidates.length > 0 && (
            <div className="border-t border-slate-800/50 pt-2">
              <p className="text-xs text-slate-500 mb-2">Candidates (Click to add):</p>
              <div className="flex flex-wrap gap-2">
                {vibeCandidates.map((candidate, i) => (
                  <button
                    key={i}
                    onClick={() => handleToggleVibeCandidate(candidate)}
                    className={`px-3 py-1 rounded-full text-xs border transition-all text-left ${song.vibe_cloud?.includes(candidate)
                        ? "bg-violet-600/30 border-violet-500 text-violet-200"
                        : "bg-slate-800/40 border-slate-700/50 text-slate-300 hover:border-violet-500/30"
                      }`}
                  >
                    {song.vibe_cloud?.includes(candidate) && <Check size={10} className="inline mr-1" />}
                    {candidate}
                  </button>
                ))}
              </div>
          ...
          <div className="flex flex-wrap gap-2 mt-2 border-t border-slate-800/50 pt-2">
            <p className="w-full text-xs text-slate-500">Selected Vibes:</p>
            {song.vibe_cloud?.map((anchor, i) => (
              <button
                key={i}
                onClick={() => handleToggleVibeCandidate(anchor)}
                className="px-3 py-1 bg-violet-900/20 hover:bg-red-900/20 hover:border-red-500/30 hover:text-red-200 border border-violet-500/30 rounded-full text-xs text-violet-200 shadow-sm transition-all cursor-pointer group"
                title="Click to remove"
              >
                {anchor} <X size={10} className="inline ml-1 opacity-0 group-hover:opacity-100" />
              </button>
            ))}
            {!song.vibe_cloud?.length && (
              <p className="text-slate-600 text-xs italic">No vibes selected.</p>
            )}
          </div>
        </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs transition-all disabled:opacity-50 flex items-center gap-2 border border-slate-700/30 ${showStress
